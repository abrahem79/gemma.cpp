...
// Double-dispatch, only for activation types: kBF16 and kF32.
template <class Func, typename... Args>
decltype(auto) CallUpcasted2(const MatPtr* base1, const MatPtr* base2,
                             const Func& func, Args&&... args) {
  const bool a_f32 = base1->GetType() == Type::kF32;
  const bool b_f32 = base2->GetType() == Type::kF32;
  HWY_ASSERT(a_f32 || base1->GetType() == Type::kBF16);
  HWY_ASSERT(b_f32 || base2->GetType() == Type::kBF16);
  if (a_f32 && b_f32) {
    return func(dynamic_cast<const MatPtrT<float>*>(base1),
                dynamic_cast<const MatPtrT<float>*>(base2),
                std::forward<Args>(args)...);
